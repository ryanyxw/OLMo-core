...
class Attention(AttentionBase):
    ...
    def sdpa(
        ...
        if self.cp_enabled:
            ...
            if self.use_sinks and self.sinks is not None:
                ...
                if self.sinks.ndim == 1:
                    S = self.sinks.numel()
                    sink_logits = (
                        self.sinks.view(1, 1, 1, S)
                        .to(attn_logits)
                        .expand(batch_size, n_heads, seq_len, S)
                    )
                elif self.sinks.ndim == 2:
                    assert self.sinks.size(0) == n_heads, "Sinks first dim must equal n_heads"
                    S = self.sinks.size(1)
                    sink_logits = (
                        self.sinks.view(1, n_heads, 1, S)
                        .to(attn_logits)
                        .expand(batch_size, n_heads, seq_len, S)
                    )
                else:
